...
MA                   = 15.0      #40 #2.5 #Alfven Mach number, defined as U_0/v_A
...
Ly                   = 10.0*np.pi
Lz                   = 10.0*np.pi
nx                   = 1024  #512 #96 #256*2 #1024  #192
ny                   = 1024    #512 #96 #256*2 #1024  #192
nz                   = 1024  #512 #96 #1024  #256 #96
mesh                 = [128, 256] #no. of processros along x and y directions. product of this should match with number of total processir asked.   
sim_time             = 4001     #301 #82.00 #148.00 #65.55 #45.01 #make this 1 more than a perfect even number so that  sim_time = integer*checkpoint_time +1.
checkpoint_time      = 5
wall_time            = 86400/24*42 #The last one hour out of 48 hours would be devoted to merge files at the end of solver calculations.
max_steps            = np.inf 
...
run_number           = 'run392'  #run1 #used to store files for separate runs in separate directory
month_of_run         = 'oct2020'
script_version_number= 'run392' # change script title as well with the change in version.
...
if restart_after_core_crash == False:
    ...
    nz_IC                = 16 #gridresolution in z-direction in IC
...
cfl        = flow_tools.CFL(solver, initial_dt=dt, cadence = 1, safety = 0.1, max_change = 1.5, max_dt = 100*dt0, threshold = 0.05) 
...
##Analysis tasks###
###################
analysis = solver.evaluator.add_file_handler('%s' %path_2, sim_dt = 10, mode = 'append') 
